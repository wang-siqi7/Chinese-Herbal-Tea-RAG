...
class KnowledgeBaseService(object):
    ...
    def upload_by_str(self, data: str, filename):
        ...
        # ③ 文本切分：手动按用户分割，每个用户独立成一个块
        import re
        # 匹配 【用户 X】 开头，直到下一个 【用户 X】 或文件结尾
        user_pattern = r'(【用户 \d+】.*?)(?=【用户 \d+】|$)'
        users = re.findall(user_pattern, data, re.DOTALL)

        if users:
            self.user_count = len(users)
            # 每个用户独立成一个块
            knowledge_chunks = users
            print(f"手动分割成功，共 {len(users)} 个用户，每个用户独立成一个块")
        else:
            # 降级：如果没有找到【用户 X】，用原来的切分方式
            if len(data) > config.max_split_char_number:
                knowledge_chunks = self.spliter.split_text(data)
            else:
                knowledge_chunks = [data]
            print(f"使用默认切分，共 {len(knowledge_chunks)} 个块")
